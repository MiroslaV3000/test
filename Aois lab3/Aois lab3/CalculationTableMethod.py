...
def print_table(minterms, is_cnf=True):
    ...
    table_data = [[prime] for prime in prime_implicants]

    # Заголовки таблицы
    headers = ["Prime Implicants"]
    ...
    for prime in prime_implicants:
        if is_cnf:
            # Для CNF, мы используем '&' и '|' для логических операций AND и OR
            output = '(' + '|'.join(
                [f"x{i + 1}" if c == '1' else f"!x{i + 1}" for i, c in enumerate(prime) if c != '-']) + ')'
        else:
            # Для DNF, мы используем '|' и '&' для логических операций OR и AND
            output = '(' + '|'.join(
                [f"x{i + 1}" if c == '1' else f"!x{i + 1}" for i, c in enumerate(prime) if c != '-']) + ')'
